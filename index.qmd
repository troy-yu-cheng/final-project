...
In this project, we use a government and policy-relevant dataset to develop a practical binary classification model predicting whether a reported crime incident in Washington, D.C. is a felony.
...
## Add felony_rate Column at Tract Level
...
crime_enriched|>
  filter()
unique(crime_enriched$census_tract)

unique(acs_processed$GEOID)

# 1. Compare crime_enriched (census_tract) with TRACTCE
...
# 2. Compare acs_raw GEOID with official GEOID
acs_tracts <- unique(acs_processed$GEOID)
...
```
官方 DC shapefile (dc_tract_sf) 中只有 206 个 tract，而 crime_enriched$census_tract 有 232 个，acs_processed$GEOID 有 231 个

* DC 的 tract 编码确实在 2010 年之后经历了少量合并、拆分、重编号的变动。尤其是 ACS 每年发布的数据可能包含历史或过渡 tract 编码，而 TIGER shapefile（如你用的 2023 年版）则只反映“最新有效”的编码。

```{r}

# 提取 unexpected_in_acs 的后六位
...
两个data frame多出来的tract编码完全重合

crime_enriched和acs_processed过滤出从2019-2023的数据，再重新跟2023 dc tract shp进行比对
...
acs_tracts_filtered <- unique(acs_filtered$GEOID)
...
# 比较这两个 tract 列表是否一致
setequal(unexpected_in_crime_2019_2023, unexpected_in_acs_tract6_filtered)

```

 结论总结：
在 2008–2025 的全量数据中：
crime_enriched 和 acs_processed 都包含 25 个 超出 2023 官方 shapefile 的 tract 编码

它们 完全重合：说明这 25 个 tract 是一致的历史遗留 tract

在 2019–2023 的数据中：
crime_enriched 的这些旧 tract 全部消失

但 acs_processed 的这些旧 tract 仍然存在

这25个 tract 在2015-2019到2019–2023的 ACS 数据中仍然存在，但已不再出现在 2023 年 DC 官方 TIGER shapefile 中，也不再在 2019–2023 的 crime 数据中出现。

```{r}
# 过滤 acs_processed 2020-2023
...
# 提取后6位进行比较
unexpected_in_acs_tract6_filtered_2020 <- substr(unexpected_in_acs_2020_2023, 6, 11)

# 比较这两个 tract 列表是否一致
setequal(unexpected_in_crime_2019_2023, unexpected_in_acs_tract6_filtered_2020)

```
2016-2020到2019-2023的acs tract数据现在和dc tract shp以及2019-2023的crime incidents完全重合了
...
This part is for final project's data and is still in processing. 
