...
```{r}
unique(crime_train$felony_severity_trans)
```
...
This model leverages all available features to predict `felony_severity_trans`, a transformed version of felony severity. It includes demographic, spatial, temporal, and contextual predictors. We again apply a Yeo-Johnson transformation to the outcome and clean predictors using normalization, correlation filtering, and dummy encoding.

```{r}
#| label: crime_rec_xgb_reg
crime_rec_xgb_reg <- recipe(felony_severity ~ ., data = crime_train) |>
  step_rm(felony_flag) |>
  step_YeoJohnson(all_outcomes()) |>  # transform skewed outcome
  step_date(report_date_parsed, features = "dow", label = TRUE) |>
  step_mutate(
    yday = lubridate::yday(report_date_parsed),
    yday_cos = cos(2 * pi * yday / 365),
...
```{r}
# Linear Regression doesn't have built-in importance, so skip
