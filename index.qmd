...
```{r}
sort(unique(crime_train$felony_severity_trans))
```
...
crime_rec_xgb_reg <- recipe(felony_severity_trans ~ ., data = crime_train) |>
...
best_xgb <- select_best(xgb_res_reg, metric = "rmse")

# Finalize and fit model
final_xgb_wf <- finalize_workflow(xgb_wf_reg, best_xgb)
final_xgb_fit <- fit(final_xgb_wf, data = crime_train)
```
