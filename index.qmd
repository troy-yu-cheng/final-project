...
Urban crime remains a pressing public‑policy challenge that threatens residents’ safety and citywide prosperity. Washington, D.C. — with its sharp neighbourhood inequalities and highly variable crime trends — offers a revealing testbed for understanding where and why serious offences emerge. To move beyond surface-level counts, this project adopts an unsupervised machine learning approach to better understand the underlying patterns of crime in the city.

A substantial body of research links environmental features and police activity to crime outcomes. Risk‑terrain studies consistently show that proximity to liquor outlets, transit nodes, and other public facilities elevates assault and robbery rates (Bowers & Johnson 2011). Hot‑spots experiments demonstrate that concentrating patrols on micro‑places produces modest but significant crime reductions that diffuse to surrounding blocks (Braga, Papachristos, & Hureau 2014). Problem‑oriented policing (POP) evaluations likewise report a small yet reliable decline in crime and disorder when officers apply the SARA cycle to diagnose and address local problems (Weisburd et al. 2010). At the same time, cost‑of‑crime analyses reveal that felonies impose orders‑of‑magnitude larger social losses than misdemeanours, making them a natural focal point for resource allocation (McCollister, French, & Fang 2010). Recent optimisation work further argues that reallocating patrol units according to deterrence elasticities could cut expected social harm by up to 15 percent (He, Li, & Zhao 2023).

Despite these insights, two notable gaps remain. First, most empirical studies rely on pre‑defined hot spots or supervised models calibrated to past incident counts, limiting their ability to capture the fluid, non‑stationary nature of urban crime. Second, few papers integrate environmental exposure, police deployment, and felony severity into a single predictive framework. To better address this issue, this project builds three different models to predict: i) the likelihood of a crime being a felony (binary classification); ii) which felony category a crime falls under (multi-classification); iii) what felony severity score of a crime new reported in DC would be (regression task). This project offers a data‑driven tool for D.C. agencies seeking to optimise deployment and reduce the social costs of violent and high‑impact offences.
...
Also, we investigate the distribution of `felony_severity`, which is used in both the multi-class classification and regression tasks.
...
Based on the table above, crimes were categorized into an ordinal scale from 0 to 7, with 0 representing non-felonies and 7 representing the most severe felony `homicide`.

The distribution of felony_severity in the training set is highly skewed. About 69% of the observations fall under severity 0, meaning they are not classified as felonies under our scheme. The most frequent felony types are lower-level ones such as `motor vehicle theft`(severity = 1) and `burglary` (severity = 2). Mid-range felonies like `robbery` (severity = 4) and `aggravated assault` (severity = 5) occur less often, while severe crimes like `sexual abuse` (severity = 6) and `homicide` (severity = 7) are extremely rare, with only a few hundred instances in total. The category with severity 3 `arson` is the rarest, appearing only 17 times.
...
ggplot(hs, aes(x = felony_severity, y = avg_hs, group = 1)) +
  ...
  geom_point() +
  labs(
    title = "Average Percent of Community with Greater than High School Education by Felony Severity",
    x = "Felony Severity",
    y = "Average Percent with Greater than High School Education in a Community"
  ) +
  theme_minimal()
```
...
Class 1 had the precision, and class 4 had the best recall. The model performed the worst on class 3, with an accuracy and precision of 0%.
...
# Set up 5-fold cross-validation
set.seed(123)
crime_folds_reg <- vfold_cv(crime_train, v = 5)
...
set.seed(123)
final_rf_fit_reg <- fit(final_rf_wf_reg, data = crime_train)
...
final_xgb_wf_reg <- finalize_workflow(xgb_wf_reg, best_xgb)

set.seed(123)
final_xgb_fit_reg <- fit(final_xgb_wf, data = crime_train)
```
...
```{r}
# XGBoost importance
xgb_model_reg <- extract_fit_parsnip(final_xgb_fit)
vip(xgb_model_reg, num_features = 20)

```

Though the importance values are more concentrated. The `method_OTHERS`, `shift_MIDNIGHT` and `black_pct` dominate, while demographic variables such as `foreign_born_pct`, `poverty_rate` and `age65plus_pct` have smaller but non-zero contributions.
...
lm_test_pred <- predict(lm_fit, new_data = crime_test) |> 
  mutate(.pred = predict(yj_obj, newdata = .pred, inverse = TRUE)) |>  # inverse transformation
  bind_cols(crime_test |> select(felony_severity))

metrics(lm_test_pred, truth = felony_severity, estimate = .pred)

# Random Forest - Test
rf_test_pred <- predict(final_rf_fit_reg, new_data = crime_test) |> 
...
# XGBoost - Test
xgb_test_pred <- predict(final_xgb_fit, new_data = crime_test) |> 
  mutate(.pred = predict(yj_obj, newdata = .pred, inverse = TRUE)) |>
  bind_cols(crime_test |> select(felony_severity))
...
After evaluating model performance on the held-out test set using RMSE, R-squared, and MAE after applying inverse transformation. Linear regression showed weak performance, with an RMSE of 1.83 and R² of 0.15, capturing little variance. Random forest performed best, with an RMSE of 1.50 and R² of 0.52. XGBoost had similar RMSE to linear regression but slightly better R² at 0.24.
...
xgb_impl_pred <- predict(final_xgb_fit, new_data = crime_enriched_implement) |> 
  mutate(.pred = predict(yj_obj_impl, newdata = .pred, inverse = TRUE)) |>
  bind_cols(crime_enriched_implement |> select(felony_severity))
...
On the 2023 implementation data, linear regression and XGBoost failed to generalize, with RMSEs of 7.06 and 6.62, and R² values near zero. In contrast, random forest maintained strong performance with an RMSE of 1.34 and R² of 0.53.
