...
73% of the crimes are classified as felony severity 0, corresponding to 'not a felony.'
...
Our first model is a random forest model using all of the predictors included in the dataset to predict the felony severity.
...
The overall accuracy rate for our random forest model is 76.68%, showing that overall, the model is strong at predicting the accurate felony severity category.

However, looking at the confusion matrix, we can see what is driving this high accuracy rate. For non-felonies (felony severity = 0), the model has a 99.24% accuracy rate:
...
Method is the most important variable, with a normalized importance of 1.
...
```
Our CART model is predicting only the majority class (not a felony; felony_severity = 0), and therefore not learning the minority classes. This is due to the class imbalance, where felony_severity = 0 makes up 73% of the dataset.

## Candidate Model 3: KNN Model

```{r}
##### original recipe #####
#knn_recipe <- recipe(felony_severity ~., data = multiclass_train) |>
  #step_normalize(all_numeric_predictors())
...
# so the model is predicting about 66% of the crime cases in D.C. in the correct felony type
